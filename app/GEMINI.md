...
├── backend/billiard_master/       # go-zero API 服务
├── theme.json                     # 主题变量源
...
- 后端以 `backend/billiard_master/billiard_master.api` 为契约源，`internal/logic -> internal/model` 承接业务与数据访问，`internal/svc/service_context.go` 统一注入 Gorm、Redis、阿里云短信、UniPush。
...
| 后端接口定义 | `backend/billiard_master/billiard_master.api` | 改完必须立刻执行 goctl |
| 后端依赖注入 | `backend/billiard_master/internal/svc/service_context.go` | model、Redis、短信、Push 初始化都在这里 |
| 业务逻辑 | `backend/billiard_master/internal/logic/` | 一接口一 logic，含少量共享 helper |
| 数据访问层 | `backend/billiard_master/internal/model/` | Gorm 模型与查询方法集中在这里 |
| 用户态上下文解析 | `backend/billiard_master/internal/utils/jwt_parser.go` | 统一用 `GetUserIDFromCtx` |
| 数据库迁移 | `backend/billiard_master/migrations/*.sql`, `backend/billiard_master/goose.sh` | 迁移和模型要同步维护 |
...
- 生成 handler、logic 和 types 代码的命令是：`goctl api go --api billiard_master.api --dir . --style go_zero --home ~/.goctl/default`。
- 当前项目下任何 `.api` 文件里的 `service` 都只能用 `service billiard_master-api`。
